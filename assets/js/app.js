...
const pintarCards = data => {
    data.forEach(item => {
        ...
        templateCard.querySelector('.precio--new').textContent = item.newprecio
...
const addCarrito = e => {
    //si esta la clase btn-dark
    if (e.target.classList.contains('btn-dark')) {
        //empujar producto 
        setCarrito(e.target.parentElement)
    }
    //detener el evento de boton agregar
    e.stopPropagation()
}
...
const setCarrito = item => {
    const producto = {
        ...
        precio: item.querySelector('.precio--new').textContent,
...
const pintarFooter = () => {
    ...
    const numero = document.querySelector(".badge-primary");
    if (nCantidad>=1){

        console.log('este',nCantidad)
        numero.innerHTML =nCantidad
    }
